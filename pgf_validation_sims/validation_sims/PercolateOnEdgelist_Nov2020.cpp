...
void percolateandtrace(vector< vector<int> > &, vector<int> &, vector<int> &, double, double, double ); //function prototype
...
int main(int argc, const char *argv[])
{
  ...
  double T = 0.3;
  double f = atof(argv[2]); //transmission probability
...
for(int net=0; net<N; ++net) {
    ...
    for(int sim=0; sim<1000; ++sim) {
        ...
        //run the epidemic
        percolateandtrace(adjmat, status, newcases, T, P, f);
...
//output
cout.precision(6);
if(nb_data_point > 0) cout << std::fixed << f << "  " << P << "  " << sum_of_data_points/nb_data_point << "  " << 1.0*nb_data_point/(1000.0*N) << endl;
else  cout << std::fixed << f << "  " << P << "  " << 0.0 << "  " << 0.0 << endl;

return 0;
} //end main

void percolateandtrace(vector< vector<int> > &neighbour, vector<int> &status, vector<int> &newcases, double T, double P, double f)
	{ 
        ...
        for(int cas=0; cas<currentcases.size(); ++cas) {
            ...
            for(int voisin=0; voisin<neighbour[currentinfected].size(); ++voisin) {
                ...
                if(status[currentneighbour]==0 && uni() < T) {
                    ...
                    double dice = uni();
                    if(!globaltrace && dice>P) {
                        possiblenewcases.push_back(currentneighbour);
                    }
                    if(!globaltrace && dice<P) {
                        globaltrace = true; 
                        for(vector<int>::iterator it = possiblenewcases.begin(); it!=possiblenewcases.end();) {if(uni()>f) it = possiblenewcases.erase(it); else ++it;}
                    }
                    if(globaltrace && uni() < f) possiblenewcases.push_back(currentneighbour);
                }
            }
            percolateandtrace(neighbour, status, possiblenewcases, T, P, f);
        }
