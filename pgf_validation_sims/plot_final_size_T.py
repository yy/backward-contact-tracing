...
def R0bar(x,y,P,R):
    ...
    for i in np.arange(1,max(R.shape)):
        summation += R[i-1]*((1-P)**i*x**i + (1-(1-P)**i)*y**i)
    return summation
...
def solve_for_S(P,T):
    ...
    while not np.allclose(u_old, u_new, rtol=1e-03, atol=1e-05):
        u_old = u_new
        R_new = R1(u_old,T,pk)
        u_new = Rbar1(u_old, 1, P, R_new)
    R1_new = R1(u_new,T,pk)
    R0_new = R0(u_new,T,pk) # Dummy value
    #return (1 - R0bar(u_new, 1, P, R0_new))*(1+yavg(P,R1_new)/xavg(P,R1_new))
    return (1 - R0bar(u_new, 1, P, R0_new))
...
data = np.loadtxt('./validation_sims/ResultsBA2_T.dat')
...
# Parameter sets
T = np.linspace(0.001, 1.0, 100)
P = np.arange(0,20,5)/100
Z = np.zeros((max(P.shape),max(T.shape)))

# Compute and plot final size
for p in np.arange(0,max(P.shape)):
    for t in np.arange(0,max(T.shape)):
        Z[p,t] = solve_for_S(P[p],T[t])
    ax.plot(T,Z[p,:], color = colors[t], label=r'$P='+str(P[p])+'$')
    y = data[data[:,0] == P[p]]
    ax.plot(y[:,0], y[:,3], mec=colors[t], ls='None', lw=3, marker='o', mfc='None', ms=3, mew=3)

# Labels
ax.set_xlabel(r'Transmission probability')
