...
data = np.loadtxt('./validation_sims/ResultsBA2.dat')
...
# Parameter sets
T = np.arange(1,10,2)/10
P = np.linspace(0.001, 1.0, 100)
Z = np.zeros((max(T.shape),max(P.shape)))

# Compute and plot final size
for t in np.arange(0,max(T.shape)):
    for p in np.arange(0,max(P.shape)):
        Z[t,p] = solve_for_S(P[p],T[t])
    ax.plot(P,Z[t,:], color = colors[t], label=r'$T='+str(T[t])+'$')
    y = data[data[:,0] == T[t]]
    ax.plot(y[:,1], y[:,3], mec=colors[t], ls='None', lw=3, marker='o', mfc='None', ms=3, mew=3)

# Labels
ax.set_xlabel(r'Contact tracing probability')
